...
{
    class MyRectangle : Figure//наследуются от Figure
    {
        int height, width;//координаты высоты и ширины
        Pen pen;//координаты пера
        public MyRectangle(Pen Pen, int X, int Y, int Height, int Width)//конструктор, для передачи параметров из класса Form1
        {
        ...
        public override void Draw(Graphics gr)//переопределенный метод отрисовки
        {
            ...
            gr.DrawRectangle(pen, rectangle);//отрисовыаем прямогольник
        }
        ...
        public override bool IsPointInside(int X, int Y)//проверка на какую фигуру тыкнули
        {
            ...
            if ((X <= x + width) && (X >= x) && ((Y <= y + height) && (Y >= y)))//проверка наши координаты лежат в нашей области прямоуг
            {
                return  true;//да
            }
